...
import { getCrystalPrompt } from '@/lib/ai/prompts/crystals'

const COST_POINTS = 7
const TOOL_ID = 'crystals'

export async function POST(req: NextRequest) {
    try {
        const { userId, intention, context } = await req.json()
        ...
        const prompt = getCrystalPrompt(intention, context)
        ...
        return NextResponse.json({ error: 'Falha ao consultar oráculo' }, { status: 500 })
    }
}
